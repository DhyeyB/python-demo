...
def test_list(user_client):
    """
    TEST CASE: Get contract logs by contract uuid
    User : Any User Type
    """
    ...
    api_response = user_client.get(
        '/api/v1/contract/list-logs/' + contract_uuid,
    ...
    data = {'result': jsonify(contract_logs_list).json}
...
def test_list_not_logged_in(user_client):
    """
    TEST CASE: (Negative) Get contract logs by contract uuid - USER not logged in
    User : Any User Type
    """
    contract_obj = Contract.get_by_id(1)
    contract_uuid = contract_obj.uuid
